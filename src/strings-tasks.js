...
function countVowels(str) {
  return str.split('').filter((c) => 'aeiouyAEIOUY'.includes(c)).length;
}
...
function isPalindrome(str) {
  ...
  return (
    str
      ...
      .split('')
      .reverse()
      .join('') === str.toUpperCase().replaceAll(re, '').replaceAll(' ', '')
  );
}
...
function findLongestWord(sentence) {
  const longestWord = sentence.split(' ').sort(function y(a, b) {
...
function invertCase(str) {
  const newStr = str.replace(/[A-Za-z]/g, function t(char) {
